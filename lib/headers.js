 ...
 */
export function addHeaders (rawHeaders, headers, context = null, logger = null) {
  const addedHeaders = []
      ...
      if (context) {
        try {
          ...
          if (compiledFgh) {
            ...
            if (logger) {
              logger.debug({ headerName, value: finalValue }, 'Added dynamic header from FGH expression')
            }
            }
          }
        } catch (err) {
          if (logger) {
            logger.error({ headerName, error: err.message }, 'Error evaluating FGH for header')
          }
