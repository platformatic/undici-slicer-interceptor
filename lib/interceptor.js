 ...
 */
export function createRequestOnlyHandler (options, originalHandler, router, logger, tmp) {
  ...
  process._rawDebug('--------SLICER REQUEST--------', path, JSON.stringify(tmp, null, 2))
 ...
 * @returns {Object} Modified handler object
 */
export function createControllerHandler (options, originalHandler, router, logger, tmp) {
  // Get the path from options
  const path = options.path || ''
  const method = options.method ? options.method.toUpperCase() : 'GET'
  ...
  if (matchingRule && doesRuleNeedResponseBodyAccess(matchingRule, logger)) {
    ...
    return createRequestOnlyHandler(options, originalHandler, router, logger, tmp)
  }
}
 ...
 */
export function createInterceptorFunction (router, logger, tmp) {
  return function slicerInterceptor (dispatch) {
    ...
    return function slicerDispatch (options, handler) {
      ...
      const wrappedHandler = createControllerHandler(options, controllerHandler, router, logger, tmp)
