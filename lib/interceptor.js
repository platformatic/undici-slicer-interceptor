import { addCacheControlHeader, addCacheTagsHeader, addHeaders } from './headers.js'
  ...
  return {
    ...
    onHeaders: function (statusCode, rawHeaders, resume, statusMessage) {
      ...
      if (matchingRule && (method === 'GET' || method === 'HEAD')) {
        // For backward compatibility, handle cacheControl first
        // This takes precedence over headers['cache-control'] if both exist
        if (matchingRule.cacheControl) {
          addCacheControlHeader(rawHeaders, matchingRule.cacheControl)
        }
        
        // Then handle headers object (new approach)
        if (matchingRule.headers) {
          addHeaders(rawHeaders, matchingRule.headers)
        }
