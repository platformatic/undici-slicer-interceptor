    ...
    if (rule.headers) {
      for (const [headerName, headerValue] of Object.entries(rule.headers)) {
        if (headerValue && typeof headerValue === 'object' && headerValue.fgh) {
          // Check if the FGH expression has already been compiled
...
function createResponseBodyHandler(rule, requestContext, originalHandler, logger) {
  ...
  return {
    ...
    // Inside this function, we should call onHeaders, onData, and onComplete.
    // If we stop, the response will never complete.
    onComplete: function(trailers) {
        // Set the response-based headers before sending the response
      try {
        // If headers were already sent (in case of error or early completion), just pass through
        logger.debug('Processing complete response body for header generation')
        if (headersSent) {
        ...
        logger.debug({ headers: Object.keys(rule.responseBodyHeaders) }, 'Processing response-based headers')
        ...
        logger.debug({ headerName }, 'Header already exists, skipping')
          continue
            ...
            if (headerValue && typeof headerValue === 'object' && headerValue.fgh) {
            try {
            // Make sure we have a compiled FGH expression
            let compiledFgh = headerValue.compiledFgh
            if (!compiledFgh) {
                  // Compile it on demand if not already compiled
                  compiledFgh = compile(headerValue.fgh)
                  headerValue.compiledFgh = compiledFgh
                  logger.debug({ headerName, expression: headerValue.fgh }, 'Compiled FGH expression on demand')
                }
                
                if (compiledFgh) {
                  try {
                    // Evaluate the FGH expression
                    logger.debug({ headerName, expression: headerValue.fgh }, 'Evaluating FGH expression with compiled function')
                    ...
                    if (Array.isArray(results)) {
                      ...
                      if (filteredResults.length > 0) {
                        ...
                        // Add empty array result as empty string
                        rawHeaders.push(headerName.toLowerCase(), '')
                        logger.debug({ headerName, value: '' }, 'Added empty array result as header')
                      }
                  ...
                  try {
                    logger.debug({ headerName, expression: headerValue.fgh }, 'Compiling FGH expression on the fly')
                    const freshCompiledFgh = compile(headerValue.fgh)
                    ...
                    if (Array.isArray(results)) {
                      ...
                      if (filteredResults.length > 0) {
                        ...
                        // Add empty array result as empty string
                        rawHeaders.push(headerName.toLowerCase(), '')
                        logger.debug({ headerName, value: '' }, 'Added empty array as header')
                      }
                    } else if (results != null) {
                      rawHeaders.push(headerName.toLowerCase(), String(results))
