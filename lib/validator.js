    ...
    for (const [headerName, headerValue] of Object.entries(rule.headers)) {
      if (headerValue && typeof headerValue === 'object') {
        if (headerValue.fgh) {
          ...
          // Check if this expression accesses the response object
          const accessesResponse = doesExpressionAccessResponse(headerValue.fgh)
          if (accessesResponse) {
            logger.debug({
              header: headerName,
              route: rule.routeToMatch,
              accessesResponse
            }, 'Valid FGH expression accessing response')
          } else {
