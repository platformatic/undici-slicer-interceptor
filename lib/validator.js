...
export function validateRules (rules, logger = abstractLogging) {
  ...
  for (const rule of rules) {
    ...
    try {
      ...
      logger.error({ route: rule.routeToMatch, err }, 'Invalid route format')
      throw new Error(`Invalid route format in rule: ${rule.routeToMatch}. ${err.message}`)
    }

    // Either headers or responseBodyTransform or both must be provided
    if ((!rule.headers || typeof rule.headers !== 'object') && (!rule.responseBodyTransform || typeof rule.responseBodyTransform !== 'object')) {
      logger.error({ route: rule.routeToMatch }, 'Invalid rule: missing headers or responseBodyTransform object')
      throw new Error('Each rule must have either a headers object or a responseBodyTransform object (or both)')
    }
      ...
      for (const [headerName, headerValue] of Object.entries(rule.headers)) {
        if (headerValue && typeof headerValue === 'object') {
          if (headerValue.fgh) {
            ...
            // Compile the FGH expression to ensure it's valid
            try {
              ...
              logger.error({ header: headerName, route: rule.routeToMatch, err }, 'Invalid FGH expression')
              throw new Error(`Error compiling FGH expression for header ${headerName}: ${headerValue.fgh}`)
            }
          } else {
    ...
    // Validate responseBodyTransform if present
    if (rule.responseBodyTransform && typeof rule.responseBodyTransform === 'object') {
      if (!rule.responseBodyTransform.fgh) {
        logger.error({ route: rule.routeToMatch }, 'Invalid responseBodyTransform: missing fgh property')
        throw new Error('responseBodyTransform must have an fgh property')
      }

      if (typeof rule.responseBodyTransform.fgh !== 'string') {
        logger.error({ route: rule.routeToMatch }, 'Invalid responseBodyTransform: fgh is not a string')
        throw new Error('responseBodyTransform.fgh must be a string')
      }

      // Compile the FGH expression to ensure it's valid
      try {
        rule.responseBodyTransform.compiledFgh = compile(rule.responseBodyTransform.fgh)
        logger.debug({ route: rule.routeToMatch }, 'Valid responseBodyTransform FGH expression')

        // Mark this rule as requiring response body access
        rule.needsResponseBodyAccess = true
      } catch (err) {
        logger.error({ route: rule.routeToMatch, err }, 'Invalid responseBodyTransform FGH expression')
        throw new Error(`Error compiling FGH expression for responseBodyTransform: ${rule.responseBodyTransform.fgh}`)
      }
    }
  }
}
 ...
 * @param {Array<{routeToMatch: string, headers?: Object, responseBodyTransform?: Object}>} rules - Rules for cache control
 * @param {Object} [logger] - Logger instance
 * @returns {Array<{routeToMatch: string, headers?: Object, responseBodyTransform?: Object}>} Sorted rules
 */
