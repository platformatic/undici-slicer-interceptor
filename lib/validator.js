...
export function validateRules (rules, logger = abstractLogging) {
  ...
  for (const rule of rules) {
    ...
    if ((!rule.headers || typeof rule.headers !== 'object') && 
        (!rule.responseBodyTransform || typeof rule.responseBodyTransform !== 'object')) {
        ...
        if (headerValue && typeof headerValue === 'object') {
          if (headerValue.fgh) {
            ...
            try {
              ...
              throw new Error(`Invalid FGH expression in header ${headerName}: ${err.message}`)
            }
          } else {
    ...
    if (rule.responseBodyTransform && typeof rule.responseBodyTransform === 'object') {
      ...
      try {
        ...
        throw new Error(`Invalid FGH expression in responseBodyTransform: ${err.message}`)
      }
    }
  }
}
