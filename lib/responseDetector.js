...
function checkIfAST_AccessesResponse(ast) {
  ...
  if (ast.type === 'MemberExpression') {
    // If it's directly accessing .response (identity.response, not object.property)
    ...
    if (ast.object && ast.object.type === 'Identity' && 
      ...
      console.log('Found .response access!')
      return true
    }
  }
...
export function doesExpressionAccessResponse(fghExpression) {
  ...
  if (responseAccessRegex.test(fghExpression)) {
    console.log(`Expression "${fghExpression}" contains .response pattern`)
    return true
  }
  
  // If regex doesn't match, no need to parse AST
  return false
...
export function doesRuleRequireResponseAccess(headers) {
  ...
  for (const [headerName, headerValue] of Object.entries(headers)) {
    if (headerValue && typeof headerValue === 'object' && headerValue.fgh) {
      ...
      if (needsResponse) {
        console.log(`Header ${headerName} with expression "${headerValue.fgh}" requires response access`)
        return true
      }
    }
  }
