...
const responseAccessRegex = /\.response(?:\.(body|headers))?\b/
...
function checkIfAST_AccessesResponse (ast) {
  ...
  if (ast.type === 'MemberExpression') {
    ...
    // Access to response.headers or response.body
    if (ast.object && ast.object.type === 'MemberExpression' &&
        ast.object.property === 'response' &&
        (ast.property === 'headers' || ast.property === 'body')) {
      return true
    }
