...
export function createRouter (rules, options) {
  ...
  for (const [origin, originRules] of rulesByOrigin.entries()) {
    ...
    for (const rule of originRules) {
      ...
      if (pathMap.get(rule.path).length > 0) {
        const existingRule = pathMap.get(rule.path)[0]
  ...
  return {
    ...
    find (method, path, options) {
      ...
      // Return the first rule (most specific one) as per the original design
      // Rules are already sorted by specificity in index.js
      const matchResult = { ...result }
      matchResult.rule = rules[0]
      return matchResult
    }
  }
}
