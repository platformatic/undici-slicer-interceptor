...
export function createRouter (rules, options) {
  ...
  for (const rule of rules) {
    ...
    rule.hadProtocol = routeInfo.hasProtocol
  ...
  return {
    ...
    find (method, path, options) {
      ...
      let matchingRule = rules.find(rule => rule.hadProtocol === effectiveHasProtocol)
      
      // If no exact protocol match is found, fall back to any rule
      if (!matchingRule) {
        // Prefer the rule without protocol first
        matchingRule = rules.find(rule => !rule.hadProtocol)
        
        // If still no match, then just use the first rule
        if (!matchingRule) {
