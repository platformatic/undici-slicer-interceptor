...
describe('make-cacheable-interceptor - headers with FGH', () => {
  ...
  // Test that the old system still works alongside the new one
  test('should support both old cacheTags property and new header-based FGH', async () => {
    // Setup test server
    const server = createServer((req, res) => {
      res.end('hello world')
    })
    ...
    try {
      // Create agent with our interceptor using both systems
      const agent = new Agent()
      const interceptor = createInterceptor([
        {
          routeToMatch: `${hostname}/both-systems/:id`,
          headers: {
            'cache-control': 'public, max-age=3600',
            'x-some-header': { fgh: "'value-' + .params.id" }
          },
          cacheTags: "'legacy', 'item-' + .params.id"
        }
      ])
      ...
      const res = await composedAgent.request({
        method: 'GET',
        origin: serverUrl,
        path: '/both-systems/456'
      })

      assert.strictEqual(res.headers['cache-control'], 'public, max-age=3600')
      assert.strictEqual(res.headers['x-some-header'], 'value-456')
      assert.strictEqual(res.headers['x-cache-tags'], 'legacy,item-456')
      await res.body.dump()
    } finally {
      server.close()
    }
